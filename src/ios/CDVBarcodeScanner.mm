...
- (void)scan:(CDVInvokedUrlCommand*)command {
    ...
    // Opts passed as JSON: { SCAN_MODE: 'QR', overlayXib: 'file' }
...
- (UIView*)buildOverlayView {
    ...
    toolbar.items = [NSArray arrayWithObjects:flexSpace,cancelButton,flexSpace,toggleFlash,shutterButton,nil];
#else
    ...
    UITextView* txtView =  [[UITextView alloc] initWithFrame:CGRectMake(0,20,self.view.frame.size.width,80)];
    [txtView setFont:[UIFont fontWithName:@"ArialMT" size:18]];
    txtView.text = @"Coloque o código QR (quadrado) da nota no centro. Sem tremer!\nClique na câmera para ligar o flash.";
    txtView.editable = NO;
    ...
    txtView.backgroundColor = [UIColor colorWithRed:1.0 green:0.8 blue:0.3 alpha:0.5];
    ...
    [overlayView addSubview: txtView];
    return overlayView;
}
...
//-------------------------------------------------------------------------
// builds the yellow box and red line
//-------------------------------------------------------------------------
- (UIImage*)buildReticleImage {
    UIImage* result;
    ...
    if (self.processor.is2D) {
        UIColor* color = [UIColor colorWithRed:1.0 green:0.8 blue:0.0 alpha:RETICLE_ALPHA];
        CGContextSetStrokeColorWithColor(context, color.CGColor);
        CGContextSetLineWidth(context, RETICLE_WIDTH);
